...
function createTables() {
  ...
  `, (err) => {
    if (err) {
      ...
      console.log('✅ Timetable entries table created/verified');
    }
  });
  ...
  `, (err) => {
    if (err) {
      console.error('Error creating class_activities table:', err.message);
    } else {
      console.log('✅ Class activities table created/verified');
    }
  });
  ...
  `, (err) => {
    if (err) {
      console.error('Error creating subject_legends table:', err.message);
    } else {
      console.log('✅ Subject legends table created/verified');
    }
  });
  ...
  `, (err) => {
    if (err) {
      console.error('Error creating department_attendance table:', err.message);
    } else {
...
function insertSampleData() {
  ...
  setTimeout(() => {
    console.log('All tables created and data inserted. You can now check phpMyAdmin.');
    db.end();
  }, 1000);
}